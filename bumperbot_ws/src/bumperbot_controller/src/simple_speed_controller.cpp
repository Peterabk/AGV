#include "bumperbot_controller/simple_speed_controller.hpp"
...
SimpleSpeedController::SimpleSpeedController(const std::string& name)
                                  : Node(name)
                                  ...
                                  , theta_(0.0)
{
    ...
    vel_sub_ = create_subscription<geometry_msgs::msg::Twist>("/bumperbot_controller/cmd_vel", 10, std::bind(&SimpleSpeedController::velCallback, this, _1));
    ...
    //child moving frame with respect to the fixed id
    ...
    //Rviz2 visualization of drawing the transformation between the fixed and dynamic frames
    transform_broadcaster_ = std::make_unique<tf2_ros::TransformBroadcaster>(*this);
    transfrom_stamped_.header.frame_id = "odom";
    transfrom_stamped_.child_frame_id = "base_footprint";

    RCLCPP_INFO_STREAM(get_logger(), "The conversion matrix is \n" << speed_conversion_);
}


void SimpleSpeedController::velCallback(const geometry_msgs::msg::Twist &msg)
{
...
void SimpleSpeedController::jointCallback(const sensor_msgs::msg::JointState & msg)
{
    ...
    RCLCPP_INFO_STREAM(get_logger(), "X : " << x_ <<" Y : " << y_ << "Orientation : "<<theta_);
    ...
    transfrom_stamped_.transform.translation.x = x_;
    transfrom_stamped_.transform.translation.y = y_;
    transfrom_stamped_.transform.rotation.x = q.x();
    transfrom_stamped_.transform.rotation.y = q.y();
    transfrom_stamped_.transform.rotation.z = q.z();
    transfrom_stamped_.transform.rotation.w = q.w();

    transfrom_stamped_.header.stamp = get_clock()->now();
    transform_broadcaster_->sendTransform(transfrom_stamped_);
}
...
int main(int argc, char* argv[])
{
  ...
  auto node = std::make_shared<SimpleSpeedController>("simple_controller");
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}